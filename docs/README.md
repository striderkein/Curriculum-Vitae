...
物流 DX SaaS「LogiGo」の開発に従事。インフラ設定以外のフルスタック開発を担当し、新機能開発、パフォーマンス改善、API 設計、テスト整備などを幅広くリード。Claude Code を使用した並列開発手法を取り入れている。
...
- **プロジェクト詳細：**
  ...
  - NestJS + Prisma でのバックエンド API 開発
  - PostgreSQL を使用したデータベース設計
  - AWS 上でのサービス運用
  - Claude Code を活用した並列開発の導入
  - 配車計画（配車板）の機能開発・改善を多数担当（強制割当・入替・配車解除、車検期日アラート、検索機能の強化、ドラッグ操作まわりの不具合修正）
  - 各種マスタ管理機能の拡充（車両・地点・荷主の項目追加、荷主と地点の紐付け UI）とデータモデル改善
  - 請求・支払まわりの機能開発をリード
    - 付帯費用の入力・集計・帳票/CSV 出力機能の設計・実装
    - 消費税対応（税区分の選択、内税/外税、端数処理）の設計・実装
    - 締日を基準とした検索・集計機能の変更設計
  - Playwright による E2E・VRT テスト基盤の立ち上げと全面移行
    - ビジュアルリグレッションテスト（VRT）と夜間自動実行の導入
    - E2E 失敗時の Issue 自動起票と AI による自動修正フローの構築
    - E2E カバレッジ計測（ダークマップ）の導入
  - Claude Code スキル群と GitHub Actions による開発プロセス自動化（コードレビュー、レビュー依頼、チケット管理システムとの双方向同期、各種通知）
  - CI/CD 改善（高速ランナーへの移行によるコスト・時間削減、マイグレーション drift 検知、VRT の安定化）
  - API レスポンス圧縮の導入などのパフォーマンス改善、監視アラームの IaC 化
  - 認証トークン期限切れに起因する本番障害の恒久対応

### サーバーフリー株式会社（2024/02 - 2025/11）

大手インフラ企業の業務システム開発に従事。バックエンド API 開発からフロントエンド改修、クラウドインフラ構築まで幅広く担当。
...
- **役割：**
  - 詳細設計、実装
- **プロジェクト詳細：**
  - Spring Boot を使用したデータ連携バッチの開発（2 名体制）
  - Spring Boot を使用した PDF 作成 REST API の開発（2 名体制）
  - jQuery を使用した業務システムのフロントエンド改修（2 名体制）
  - Spring Boot API の Azure App Service へのデプロイ（2 名体制）
- **離職理由**
  - 一身上の都合

### 株式会社ゼヒトモ（2022/08 - 2023/08）

中小事業者と個人顧客とのマッチングサービスの開発に従事。
ユーザーの利便性を高める追加機能の設計から実装を主導。得意とするフロントエンド開発の経験を活かして、フロントエンドの改善活動をリード。特に、スマホアプリ開発で得た知見を生かしたレスポンシブ対応を主導。
...
40,000 人のユニークユーザーがいるサイトで、オンラインレッスンアプリのリードエンジニアを努めた。
WebRTC を使用し、オンラインレッスンに使用するためのチャット、ビデオミーティング、お絵描き等を実現する Web アプリの開発に従事。
